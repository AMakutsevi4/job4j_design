...
(
    ...
    duration     INTEGER,
    budget       NUMERIC(15, 2),
    company_id   INTEGER REFERENCES film_companies5 (id),
    director_id  INTEGER REFERENCES directors5 (id)
);

INSERT INTO film_companies5 (name, country, founding_year)
VALUES ('Company A', 'USA', 1990),
       ('Company B', 'UK', 1985),
       ('Company C', 'Canada', 2000);

INSERT INTO directors5 (name, birth_date, nationality)
VALUES ('Director X', '1970-05-15', 'USA'),
       ('Director Y', '1982-08-22', 'UK'),
       ('Director Z', '1965-12-10', 'Canada'),
       ('Director W', '1978-03-30', 'USA');

INSERT INTO movies5 (title, release_year, duration, budget, company_id, director_id)
...
FROM
     movies5
JOIN
         film_companies5 f on  movies5.company_id = f.id
JOIN
         directors5 d ON movies5.director_id = d.id
GROUP BY
    f.name
HAVING
    AVG(budget) > 5000000 AND
       COUNT(d.id) > 3
