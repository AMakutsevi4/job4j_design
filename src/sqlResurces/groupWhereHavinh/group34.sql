...
(
    ...
    duration     INTEGER,
    budget       NUMERIC(15, 2),
    company_id   INTEGER REFERENCES film_companies3 (id),
    director_id  INTEGER REFERENCES directors3 (id)
);


INSERT INTO film_companies3 (name, country, founding_year)
VALUES ('Company A', 'USA', 1990),
       ('Company B', 'UK', 1985),
       ('Company C', 'Canada', 2000);

INSERT INTO directors3 (name, birth_date, nationality)
VALUES ('Director X', '1970-05-15', 'USA'),
       ('Director Y', '1982-08-22', 'UK'),
       ('Director Z', '1965-12-10', 'Canada'),
       ('Director W', '1978-03-30', 'USA');

INSERT INTO movies3 (title, release_year, duration, budget, company_id, director_id)
...
FROM
     movies3
JOIN
         directors3 d ON movies3.director_id = d.id
GROUP BY
         name
HAVING
       COUNT(title) > 3;
