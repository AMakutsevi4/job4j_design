...
(
    ...
    category_id INTEGER REFERENCES categories11 (id)
);

INSERT INTO categories11 (name)
VALUES ('Электроника'),
       ('Одежда'),
       ('Бытовая техника'),
       ('Книги'),
...
INSERT INTO products11 (name, quantity, price, category_id)
...
SELECT c.id, c.name
FROM categories11 c
         JOIN products11 p ON c.id = p.category_id
WHERE p.quantity < 20 AND p.price > 500;